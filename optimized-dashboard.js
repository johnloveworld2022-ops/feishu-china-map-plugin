    ...
    if (result.data && result.data.code === 0) {
      ...
      const validData = records
        ...
        .map(record => {
          ...
          // 标准化省份名称
          if (province.includes('浙江')) province = '浙江';
          else if (province.includes('广东')) province = '广东';
          else if (province.includes('上海')) province = '上海';
          else if (province.includes('北京')) province = '北京';
          else if (province.includes('江苏')) province = '江苏';
          else if (province.includes('山东')) province = '山东';
          
          // 根据区域设置数值
          let value = 1;
          const region = fields['管理区域'];
          if (region === '华东') value = 5;
          else if (region === '华南') value = 3;
          else if (region === '华北') value = 4;
          else if (region === '华中') value = 2;
...
function createOptimizedDashboard(data) {
...
<body>
    <div class="container">
        ...
        <div class="update-time">
            📅 数据更新时间: ${new Date().toLocaleString('zh-CN')} | 🔄 实时同步飞书多维表格
        </div>
    </div>
                            ...
                            if (params.data) {
                                ...
                                return \`
                                    <div style="padding: 15px; max-width: 300px;">
                                        <div style="font-weight: bold; margin-bottom: 12px; font-size: 16px; color: #fff;">
                                            🏛️ \${data.name}
                                        </div>
...
async function startOptimizedDashboard() {
  console.log('🚀 启动优化版飞书银行机构地图仪表盘...\n');
  ...
  // 2. 生成优化版仪表盘
  console.log('🎨 生成优化版地图仪表盘...');
  const htmlContent = createOptimizedDashboard(processedData);
  ...
  server.listen(port, () => {
    ...
    console.log(`🗺️ 覆盖省份: ${processedData.map(d => d.province).join(', ')}`);
    console.log(`👥 负责人: ${[...new Set(processedData.flatMap(d => d.managers))].join(', ')}`);
    console.log(`\n💡 在浏览器中打开上述地址查看你的银行机构地图仪表盘！`);
...
if (require.main === module) {
  startOptimizedDashboard();
}

module.exports = { startOptimizedDashboard, fetchAndProcessData };